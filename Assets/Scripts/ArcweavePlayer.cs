...
namespace Arcweave
{
    public class ArcweavePlayer : MonoBehaviour
    {
        ...
        public bool HasSaveInSlot(int slot) => HasSaveInSlotStatic(slot);

        public (string label, string time) GetSlotInfo(int slot) => GetSlotInfoStatic(slot);

        public static bool HasSaveInSlotStatic(int slot) =>
            PlayerPrefs.HasKey(SlotKey(slot) + "_currentElement");

        public static (string label, string time) GetSlotInfoStatic(int slot)
        {
            var prefix = SlotKey(slot);
            return (
